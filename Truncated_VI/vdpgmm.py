...
'''
class VDPGMM(object):
    def __init__(self, T, max_iter = 100, alpha = 1, thresh=1e-5, verbose=False):
        self.T = T
    ...
    def _initialize(self, X):
        ...
        # self.mean_mu = KMeans(n_clusters=self.T).fit(X).cluster_centers_[::-1] # use kmeans to initialize the means
        # self.cov_mu = np.empty((self.T, self.P, self.P))
        # for i in range(self.T):
        #     self.cov_mu[i] = np.eye(self.P)

        #self.mean_mu = np.mean(self.X, axis = 0)
        self.mean_mu = np.zeros((self.T, self.P))
        ...
        print(self.mean_mu, self.cov_mu)
