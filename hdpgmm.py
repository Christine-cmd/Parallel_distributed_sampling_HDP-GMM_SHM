...
class Gaussian: # a variable of the Gaussian class represents a component in the HDP-GMM model, X denotes the data assigned to this component
    def __init__(self, X=np.zeros((0, 2)), kappa_0=1., nu_0=1.0001, mu_0=None, Psi_0=None):
        self.nk = X.shape[0]
        self.dim = X.shape[1]
...
class GibbsSampler(object):
    ...
    def initialize(self, data):
        ...
        # initialize the total number of topics. the number of topics is the number of global clusters
        self._K = self._D
        ...
        # initialize the table count matrix indexed by topic id, i.e., m_{\cdot k} # number of tables assigned to topic k
        self._m_k = np.zeros(self._K)

        # assignment variables, these variables are recorded using dictionaries {d:x_di}
    ...
    """
    def sample_concen(self, alpha_a=100., alpha_b=100., gamma_a=100., gamma_b=100., max_iter_alpha=20, max_iter_gamma=20):
    ...
    def sample(self, iteration, concentration_parameter=None, hyper_prior=None):
        ...
        while iter < iteration:
            iter += 1
            if concentration_parameter is not None: # user-specified alpha and gamma
                self._alpha = concentration_parameter['alpha']
                self._gamma = concentration_parameter['gamma']

            else: # sample alpha and gamma from gamma prior
                # sample new w_alpha and s_alpha
                if hyper_prior is not None:
                    self.sample_concen(hyper_prior['alpha_a'], hyper_prior['alpha_b'], hyper_prior['gamma_a'], hyper_prior['gamma_b'])
                else:
                    self.sample_concen()
                # for iter_alpha in range(max_iter_alpha):
                #     self._w_alpha = np.array([np.random.beta(self._alpha+1, np.sum(self._n_dt[j])) for j in range(self._D)])
            ...
            print("Gibbs sampling iteration %d ..." % iter, 'alpha:', self._alpha, 'gamma:', self._gamma)
            ...
            for document_index in np.random.permutation(range(self._D)):
                ...
                for word_index in np.random.permutation(range(len(self._corpus[document_index]))):
                    ...
                    # compute the log-likelihood fk_-xji(xji), self._K = number of global components
                    ...
                    for t in range(len(self._k_dt[document_index])):
                        if self._n_dt[document_index][t] > 0: # number of words (data points) in document (subset) [document_index] assigned to table (local component) t
                            ...
                            assigned_topic = int(self._k_dt[document_index][t]) #  global assignment (table t in document [document_index] assigned to topic [assigned_topic])
                    ...
                    # if current word sits on a new table, we need to get the topic of that table, if t_ji = t_new, sample a global component k_jt for t_ji (as it is a new table)
                    if new_table == len(self._k_dt[document_index]):
                        ...
                        new_topic = np.uint8(np.nonzero(cdf >= np.random.random())[0][0]) # sample a topic (global component), which can be either an existing topic or a new topic
                ...
                # sample table assignment, see which topic it should belong to # sample k_jt (global assignment)
